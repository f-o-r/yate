...
yate.asts.block.w_deinclude = function(options) {
    ...
    this.p.Items.iterate(function(item) {
        ...
        opts.filename = item.p.Filename;
        opts.input = options.resolve && options.resolve(item.p.Filename);
        var ast = yate.parse(opts);
...
yate.asts.block.w_deimport = function(options) {
    var a = [];
    var imports = [];

    var provide = options.provide || function(name) {
        return (options.modules || yate.modules)[name];
    };
    ...
    this.p.Items.iterate(function(item) {
        ...
        for (var i = 0, l = defs.length; i < l; i++) {
            var def = defs[i];
            var ast = yate.AST.fromJSON(def, null);
...
yate.asts.inline_function.w_prepare = function() {
    ...
    if (def.f.IsExternal) {
        ...
        args.iterate(function(arg, i) {
            if (defArgs.length <= i) {
                arg.error('too many arguments for the function');
            }
